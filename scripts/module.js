import {
  createSpellSlotOptions,
  renderUsePowerDialog,
} from './power-use-dialog.js';
...
let renderAbilityUseDialogHookId = Hooks.on(
...
Hooks.once('ready', () => {
  ...
  if (!game.modules.get('lib-wrapper')?.active && game.user.isGM) {
    ui.notifications.error(
      `Module ${KEY} requires the 'libWrapper' module. Please install and activate it.`
    );
  } else {
    libWrapper.register(
      KEY,
      'game.dnd5e.applications.actor.ActorSheet5eCharacter.prototype._prepareSpellbook',
      getSpellbook,
      'WRAPPER'
    );
    libWrapper.register(
      KEY,
      'game.dnd5e.applications.item.AbilityUseDialog._createSpellSlotOptions',
      createSpellSlotOptions,
      'WRAPPER'
    );
  }
...
Hooks.on('renderActorSheet5eCharacter', async (sheet, html, data) => {
  log.debug(
    'renderActorSheet5eCharacter. sheet:',
    sheet,
    'html:',
    html,
    'data:',
  ...
  if (lastUpdatedStrainActorId == data.actor.id) {
    log.debug(
      'last updated strain actor is this actor. sheet:',
      sheet,
      'html:',
      html,
      'data:',

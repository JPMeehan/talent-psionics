...
function createPowerOrderOptions(actor, order) {
  ...
  log.debug(
    'createPowerOrderOptions: Actor is a talent manifesting >1st order'
  );
...
export const renderUsePowerDialog = function (dialog, html) {
  if (dialog.item.system.preparation.mode !== 'talent') {
    if (dialog.item.actor.classes.talent !== undefined) {
      ...
      html
        .find(`form div.form-fields select[name="slotLevel"] option`)
        .map((i, e) => {
          const option = $(e);
  ...
  html
    .find("form div label.checkbox input[name='consumeSpellSlot']")
    .removeAttr('checked')
    .parent()
    .hide();
  ...
  let newContent = dialog.data.content
    .replace(castLevel, manifestAtOrder)
    .replace(checkboxRegex, '$& hidden')
    .replace(consumeSlotRegex, '');
